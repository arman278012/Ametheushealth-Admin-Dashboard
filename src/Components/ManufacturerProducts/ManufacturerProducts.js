  ...
  const getManufacturersProducts = async (page = currentPage, search = "") => {
    try {
      const response = await axios.get(
        `https://api.assetorix.com/ah/api/v1/manufacturer/admin/${manufacturerId}?page=${page}&limit=${pageLimit}&search=${search}`,
        {
          headers: {
  ...
  const deleteManufacturerProducts = async (id) => {
    try {
      const response = await axios.patch(
        `https://api.assetorix.com/ah/api/v1/manufacturer/rmid/${id}`,
        {},
        {
          headers: {
            authorization: `Bearer ${localStorage.getItem("authorization")}`,
