...
const ApproveDoctor = () => {
    ...
    const updateDoctorStatus = async (doctorId, status, statusDetail) => {
        try {
            const response = await fetch(
                `https://api.assetorix.com/ah/api/v1/dc/admin/promoted/add/${doctorId}`,
                {
                    method: "POST",
                    headers: {
    ...
    return (
        <div className="flex flex-col sm:flex-row">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-wrap gap-6">
                        {pendingDoctorList.length > 0 ? (
                            ...
                            <div>
                                <h2 className="text-center font-bold">No doctor data is available</h2>
                            </div>
                        )}
                    </div>
                )}
            ...
            <div className="sm:hidden block">
                <MobileNavbar />
                <div className="p-4">
                    <p className="text-gray-600">Please use a larger screen to manage doctors.</p>
                </div>
            </div>
        </div>
    );
